...
class Member(AbstractBaseUser, PermissionsMixin, BaseModel):
    """
    Custom user model for clan members.
    STATUS FIELD is single source of truth:
    - INVITED: Invited but hasn't registered
    - PENDING: Registered, awaiting approval
    - ACTIVE: Approved and can access system
    - BLOCKED: Explicitly blocked by moderator
    - SUSPENDED: Temporarily suspended
    - REMOVED: Permanently removed from clan
    """
    ...
    status     = models.CharField(
        ...
        db_index=True,
        help_text="Single source of truth for account lifecycle"
    )
    ...
    has_accepted_terms = models.BooleanField(default=False, help_text="Has accepted current terms of service")
    ...
    @property
    def is_active(self):
        """Django convention: True if account can be used."""
        return self.status in [MemberStatus.ACTIVE, MemberStatus.PENDING]
    ...
    def is_suspended(self):
        """Convenience property: equivalent to status == SUSPENDED."""
        return self.status == MemberStatus.SUSPENDED
    ...
    @property
    def is_leader(self):
        """A member is a leader if they have an active Leader role OR are superuser."""
        if self.is_superuser:
            return True
    ...
    def is_treasurer(self):
        """A member is a treasurer if they have an active Treasurer role OR are superuser."""
        if self.is_superuser:
            return True
    ...
    def is_secretary(self):
        """A member is a secretary if they have an active Secretary role OR are superuser."""
        if self.is_superuser:
            return True
    ...
    def is_moderator(self):
        ...
        FIXED: Removed duplicate definition that was shadowing the original.
        Now uses consistent hierarchy-level check: ELDER (1) or higher.
        """
        if self.is_superuser:
            return True
